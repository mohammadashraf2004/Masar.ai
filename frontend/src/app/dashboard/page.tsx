...
import { Enrollment, SkillScore, RoadmapWeek } from '@/types'
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            {
              label: 'This week',
              value: roadmap[0]?.theme ?? '—',
              sub: roadmap[0] ? 'Current focus' : 'Generate roadmap',
              icon: Clock,
              color: 'text-violet',
            },
            ...
            <Card key={label} glow className="p-5">
              ...
              <div className={`text-2xl font-display font-700 ${color} mb-0.5`}>{value}</div>
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="col-span-2 space-y-4">
            <h2 className="text-xs font-medium text-ghost uppercase tracking-widest">
              Your tracks
            </h2>
            {loading ? (
              <Card className="p-6 text-center text-ghost text-sm">Loading…</Card>
                ...
                <Card key={en.id} glow className="p-5">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      {en.target_job_title && (
                        <Badge variant="ghost">{en.target_job_title}</Badge>
                      )}
                    </div>
                    <span className="text-sm font-mono text-amber">
                      {en.completion_percentage.toFixed(0)}%
                    </span>
                  </div>
            ...
            {roadmap.length > 0 && (
              <div className="mt-2">
                <h2 className="text-xs font-medium text-ghost uppercase tracking-widest mb-3">
                  Weekly plan
                </h2>
          ...
          <div className="space-y-4">
                  ...
                  {skills.map(s => (
                    <div key={s.skill}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className={`text-xs font-mono ${scoreColor(s.score)}`}>
                          {s.score.toFixed(0)}%
                        </span>
                      </div>
                      <ProgressBar
            ...
            <Card className="p-5 bg-gradient-to-br from-amber/5 to-transparent border-amber/20">
              ...
              <p className="text-xs text-dim mb-4 leading-relaxed">
                Your personal mentor is ready to explain concepts, quiz you, or review your code.
              </p>
              <Link href="/mentor">
